...
**Стек:** FastAPI · SQLAlchemy 2.0 (async) · asyncpg · Alembic · aio-pika (RabbitMQ) · PostgreSQL · Docker
...
```
Booking Service  ──REST──►  Payment Service  ──RabbitMQ──►  Booking Service
...
- Booking вызывает Payment синхронно через REST — потому что ему нужно сразу получить `payment_url` и отдать его пользователю.
...
```json
{
  ...
  "amount": 1500.00,
  "currency": "RUB",
  "payment_url": "http://localhost:8000/payments/pay_a1b2c3d4",
